class AddFeaturedToPhotos < ActiveRecord::Migration
  ...
  # @residential_picture.update_attributes(featured: true) if @residential_picture.present?
  #
  # @commercial_picture = Picture.where(market: 'commercial').first
  # @commercial_picture.update_attributes(featured: true) if @commercial_picture.present?
end
